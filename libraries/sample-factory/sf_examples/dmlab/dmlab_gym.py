...
class DmlabGymEnv(gym.Env):
    def __init__(
        ...
        self.eval_info_obs = [
            'DEBUG.POS.TRANS',
            'DEBUG.POS.TRANS_VELOCITY',
            'DEBUG.POS.ROT',
            #'DEBUG.POS.ROT_VELOCITY', this doesn't seem to work :(
        ]
        if self.is_eval:
            observation_format += self.eval_info_obs
    ...
    def reset(self, **kwargs):
        ...
        return self.last_observation, {'settings': None}
    ...
    def step(self, action):
        ...
        info_step = {}
        if not terminated:
            raw_obs = self.dmlab.observations()
            obs_dict = self.format_obs_dict(raw_obs)
            self.last_observation = obs_dict

            if self.is_eval:
                for k in self.eval_info_obs:
                    debug_obs = raw_obs[k]
                    for i, data in enumerate(debug_obs):
                        info_step[f"{k}:{i}"] = data
                #print(info_step)

        info = {"num_frames": self.action_repeat, "step": info_step, "settings": None}
