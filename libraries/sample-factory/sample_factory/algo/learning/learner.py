...
class Learner(Configurable):
    ...
    def init(self) -> InitModelData:
        ...
        optimizer_cls = dict(adam=torch.optim.Adam, adamw=torch.optim.AdamW)#, lamb=Lamb)

        if self.cfg.optimizer == 'adam' and self.cfg.adam_weight_decay > 0:
            for i in range(10):
                print('Warning: Using Adam with a weight decay > 0. Did you mean to use AdamW?')
        ...
        fresh_start: bool = self.env_steps == 0   # Indirect way to check if this is a new run
        if (self.cfg.initialise_with_checkpoint != '') and fresh_start:
            assert not self.cfg.with_pbt

            log.info(f'This model is a fresh start, initialising from {self.cfg.initialise_with_checkpoint}')
            self._load_state(torch.load(self.cfg.initialise_with_checkpoint, map_location=self.device), load_progress=True)
    ...
    def _load_state(self, checkpoint_dict, load_progress=True):
        ...
        for param in self.optimizer.param_groups:
            log.warning(f'Weight decay in {self.optimizer} is {param["weight_decay"]}, overriding to {self.cfg.adam_weight_decay}')
            param['weight_decay'] = self.cfg.adam_weight_decay
    ...
    def _calculate_losses(
        ...
        self.reg_rnn_states: torch.Tensor = core_outputs  # Used for activity-based regularisation
        ...
        for epoch in range(self.cfg.num_epochs):
            ...
            for batch_num in range(len(minibatches)):
                ...
                with timing.add_time("losses_postprocess"):
                    ...
                    if self.cfg.l1_weight > 0:
                        l1_loss = self.cfg.l1_weight * sum([p.abs().sum() for p in self.actor_critic.parameters()])
                        loss += l1_loss
                        log.info(f'Total: {loss:.5f}, Actor: {actor_loss:.5f}, Critic: {value_loss:.5f}, l1: {l1_loss:.5f}')
    ...
    def train(self, batch: TensorDict) -> Optional[Dict]:
        ...
        if num_invalids >= experience_size:
            ...
            # Make sure we save the randomly initialialised model
            #if (self.cfg.save_every_step != 0) and (self.train_step == 0):
            #    self.save_milestone()

            with self.timing.add_time("train"):
                train_stats = self._train(buff, self.cfg.batch_size, experience_size, num_invalids)

            if (self.cfg.save_every_step != 0) and ((self.train_step % self.cfg.save_every_step) == 0):
                self.save_milestone()
