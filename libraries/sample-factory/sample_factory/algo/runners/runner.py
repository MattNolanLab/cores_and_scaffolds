...
def initialise_n_episodes(eventfile_path: str) -> int:
    from tensorboard.backend.event_processing.event_accumulator import EventAccumulator
    event_accumulator = EventAccumulator(eventfile_path)
    event_accumulator.Reload()

    try:
        events = event_accumulator.Scalars('episode/episode')
        n_episodes: int = int(max([e.value for e in events]))  # Maybe overkill but maybe events aren't sported
        print(f'Resuming with n episodes {n_episodes}')
        return n_episodes
    except KeyError:
        print('Episode key was not found. Initialising to 0')
        return 0
...
class Runner(EventLoopObject, Configurable):
    def __init__(self, cfg, unique_name=None):
        ...
        self.total_episodes: int = initialise_n_episodes(self.writers[0].logdir)
    ...
    def _report_experiment_summaries(self):
        ...
        for policy_id, env_steps in self.env_steps.items():
            ...
            writer.add_scalar(f"episode/episode", self.total_episodes, env_steps)
            ...
            for key, stat in self.policy_avg_stats.items():
                if len(stat[policy_id]) >= stat[policy_id].maxlen or (
                    ...
                    writer.add_scalar(f'{avg_tag}_across_episodes', float(stat_value), self.total_episodes)
