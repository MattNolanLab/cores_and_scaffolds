...
class MultiInputEncoder(Encoder):
    def __init__(self, cfg: Config, obs_space: ObsSpace):
        ...
        for obs_key in self.obs_keys:
            ...
            if len(shape) == 1:
                if cfg.use_identity_encoder:
                    log.debug('Using IdentityEncoder instead of mlp')
                    encoder_fn = IdentityEncoder
                else:
                    if cfg.rnn_type in ['lsnnpoisson', 'lifcurrent', 'rnn_relu']:
                        log.warning('Using an snn. Consider using an identity encoder')
...
class IdentityEncoder(Encoder):
    def __init__(self, cfg: Config, obs_space: ObsSpace):
        ...
        self.encoder_out_size = obs_space.shape[0]
