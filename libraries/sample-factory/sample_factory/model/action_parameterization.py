...
class ActionParameterizationDefault(ActionsParameterization):
    ...
    def __init__(self, cfg, core_out_size, action_space, bias):
        super().__init__(cfg, action_space)

        num_action_outputs = calc_num_action_parameters(action_space)
        self.distribution_linear = nn.Linear(core_out_size, num_action_outputs, bias)

    def forward(self, actor_core_output, params_only: Optional[bool] = False):
        ...
        if params_only:
            return action_distribution_params  # To support Captum hooks
        else:
