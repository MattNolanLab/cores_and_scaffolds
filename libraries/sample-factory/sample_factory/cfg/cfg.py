...
def add_rl_args(p: ArgumentParser):
    ...
    # optimization
    p.add_argument("--optimizer", default="adam", type=str, choices=["adam", "lamb", "adamw"], help="Type of optimizer to use")
    p.add_argument(
        "--adam_eps",
        default=1e-6,
        type=float,
    ...
    p.add_argument("--adam_weight_decay", default=0.0, type=float, help="Adam weight decay")
    p.add_argument(
        "--max_grad_norm",
        default=4.0,
        type=float,
    ...
    p.add_argument("--save_every_step", default=0, type=int, help="Save a model every n training steps (including at initialisation)")
    ...
    p.add_argument(
        ...
        default=0.0,
        type=float,
        help="Scale of additive normal noise to add to recurrent state",
    )
    p.add_argument(
        "--recurrent_dropout",
        default=0.0,
        type=float,
        help="Probability to drop an element of the hidden state",
    )
...
def add_model_args(p: ArgumentParser):
    ...
    p.add_argument("--use_identity_encoder", default=False, type=str2bool, help="Use an identity encoder (instead of MLP)")
    ...
    p.add_argument("--rnn_l2_weight_decay", default=0.0, type=float, help="L2 weight decay applied only to RNN")
    p.add_argument("--l1_weight", default=0.0, type=float, help="L1 regualrisation penalty applied to whole model")
    p.add_argument(
        "--rnn_size",
        default=512,
        type=int,
    ...
    p.add_argument(
        "--rnn_type",
        default="rnn_relu_bias",
        choices=["rnn_relu", "rnn_tanh", "rnn_tanh_bias", "rnn_relu_bias"],
        type=str,
        help="Type of RNN cell to use if use_rnn is True",
    )
    p.add_argument(
        "--parametric_rnn_layers",
        default=0,
        type=int,
        help="Number of layers to use in parametric RNN",
    )
    p.add_argument(
        "--parametric_rnn_branches",
        default=0,
        type=int,
        help="Number of branches to use in parametric RNN",
    )
    p.add_argument(
        "--parametric_rnn_bleed",
        default=0,
        type=int,
        help="Number of outputs per branch that bleed into the downstream branch",
    )
    ...
    p.add_argument(
        ...
        default=False,
        type=str2bool,
        help="Force the critic to use GRU cells even if the actor uses a different cell",
    )
    p.add_argument(
        "--initialise_with_checkpoint",
        default='',
        type=str,
        help="Path to .pth if want to initialise the network ",
    )
    ...
    p.add_argument("--critic_bias", default=True, type=str2bool, help="Enable bias on the critic")
    p.add_argument("--actor_bias", default=True, type=str2bool, help="Enable bias on the actor")
    p.add_argument(
    ...
    p.add_argument(
        ...
        choices=["torch_default", "esn"],
        type=str,
        help="RNN weight initialization",
    )
    p.add_argument(
        "--policy_init_gain",
        default=1.0,
    ...
    p.add_argument(
        "--action_init_magnitude_scale",
        default=1.0,
        type=float,
        help="Magnitude scaling factor for the action readout (see Ostojic/Barak paper about aligned/oblique dynamics). 1.0 == untouched",
    )
    p.add_argument(
