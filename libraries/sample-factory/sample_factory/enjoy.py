...
def enjoy(cfg: Config) -> Tuple[StatusCode, float]:
    ...
    try:
        reward_avg = sum([sum(episode_rewards[i]) for i in range(env.num_agents)]) / sum(
            [len(episode_rewards[i]) for i in range(env.num_agents)]
        )
    except ZeroDivisionError:
        reward_avg = np.nan

    return ExperimentStatus.SUCCESS, reward_avg


def enjoy_with_data(cfg: Config,
                    milestone_path_override: Optional[Path]=None,
                    core_override: Optional[ModelCore]=None,
                    distribution_linear_override: Optional[torch.nn.Linear]=None,
                    decoder_override: Optional[MlpDecoder]=None,
                    ) -> Tuple[StatusCode, float, Dict[Any, Any], torch.nn.Module]:
    """Adapted from sample-factory enjoy.py"""
    verbose = False
    ...
    assert env.num_agents == 1, "Num agents > 1 not supported"
    ...
    if milestone_path_override is None:
        policy_id = cfg.policy_index
        name_prefix = dict(latest="checkpoint", best="best")[cfg.load_checkpoint_kind]
        checkpoints = Learner.get_checkpoints(Learner.checkpoint_dir(cfg, policy_id), f"{name_prefix}_*")
    else:
        log.info(f'Loading from {milestone_path_override} instead of the normal model')
        checkpoints = [str(milestone_path_override)]

    checkpoint_dict = Learner.load_checkpoint(checkpoints, device)
    actor_critic.load_state_dict(checkpoint_dict["model"])

    if core_override is not None:
        log.debug('Replacing loaded core with user-provided core. User is probably performing RNN weight perturbation experiments. This is only supported for simple single-core RNN models.')
        actor_critic.core.core = core_override
        actor_critic.core.core.to(device)

    if distribution_linear_override is not None:
        print('Replacing linear distribution readout. User is probably performing readout perturbation experiments.')
        actor_critic.action_parameterization.distribution_linear = distribution_linear_override.to(device)

    if decoder_override is not None:
        print('Replacing decoder. User is probably performing indirect action readout perturbations')
        actor_critic.decoder = decoder_override.to(device)
    ...
    data_to_save = {
        ...
        'rewards': [],
        'done_or_terminated': [],
        'info_settings': deepcopy(infos[0]['settings'])
    ...
    with torch.no_grad():
        while not max_frames_reached(num_frames):
            ...
            if not cfg.no_render:
                visualize_policy_inputs(normalized_obs)
            policy_outputs = actor_critic(normalized_obs, rnn_states, perturbation_metadata=data_to_save)
            ...
            for _ in range(render_action_repeat):
                ...
                data_to_save['rnn_states'].append(rnn_states.cpu())
                data_to_save['rnn_inputs'].append(actor_critic.forward_head(normalized_obs).cpu())

                data_to_save['info_step'].append(deepcopy(infos[0]['step']))
                # Now add data to the most recent info step dict
                data_to_save['info_step'][-1]['timestep'] = timestep
                data_to_save['info_step'][-1]['global_timestep'] = num_frames
                data_to_save['info_step'][-1]['episode'] = num_episodes
                data_to_save['info_step'][-1]['done_or_terminated'] = dones[0].item()

                data_to_save['rewards'].append(rew.float())
                ...
                num_frames += 1
                if num_frames % 100 == 0:
                    log.debug(f"Num frames {num_frames}... Episode timestep {timestep}")
    ...
    total_reward: float = sum([sum(episode_rewards[i]) for i in range(env.num_agents)])
    n_episodes = sum([len(episode_rewards[i]) for i in range(env.num_agents)])
    if n_episodes == 0:
        avg_reward: float = total_reward
        print('Warning: Agent did not finish any episodes. Reward calculation is based on a single incomplete episode')
    else:
        avg_reward: float = total_reward / n_episodes

    return ExperimentStatus.SUCCESS, avg_reward, data_to_save, actor_critic
